...
network_builder::construct(const network_properties &networkProperties)
{
   ...
   std::unique_ptr<int_network> networkProduct {};
   ...
   const bool allowLoops) const
{
   ...
   const bool connections_full {std::all_of(freeDegreePdf.begin(),freeDegreePdf.end(),[] (const degree d)
   {
      return d == 0U;
   })};
   ...
   const bool allowLoops) const
{
   if(!allowLoops)
   {
      return is_possible_to_create_connection_without_loops(freeDegreePdf);
   }
   else
   {
      return is_possible_to_create_connection_with_loops(freeDegreePdf);
   }
}

void network_builder::create_random_connection(
   const int_node_list &nodes, std::vector<degree> &freeDegreePdf, const bool allowLoops) const
{
   ...
   // the freeDegreePdf has to be reset, if the connection is not made

   if(firstNodeId != secondNodeId || (firstNodeId == secondNodeId && allowLoops))
   {
      // creating a connection between first node and second node
      LOG(DEBUG, "Creating connection between nodes ", firstNodeId, " and ", secondNodeId);
