...
void network::set_nodes(int_node_list &nodes) { mNodes = std::move(nodes); }

void network::set_transmissibility(const double transmissibility) { mTransmissibility = transmissibility; }

const int_node_list &network::get_nodes() const { return mNodes; }

event_list network::inject_information_to_random_node(const time now) {
  std::random_device randomDevice;
  std::mt19937 randomNumberGenerator(randomDevice());
  std::uniform_int_distribution<int> distribution(0, get_num_of_nodes() - 1U);
...
event_list network::inform_node(const node_id nodeId, const time now) {
  ...
  event_list events = mNodes[nodeId]->inform(now, mTransmissibility);

  for (event event : *events) {
    LOG(DEBUG, "Generated events: from ", event.mFromNodeId, " to ",
        event.mToNodeId, " at time ", event.mTime);
  }
...
size_t network::get_num_of_nodes() const { return mNodes.size(); }

std::unique_ptr<result> network::get_result() const {
  auto simualtionResult = std::make_unique<result>();

  for (const auto &node : mNodes) {
    simualtionResult->push_back(
        result_record{node->get_information_time().value_or(
  ...
  const auto iteratorToFirstNan = std::partition(
      ...
      [](const auto &record) { return !std::isnan(record.mInformationTime); });
